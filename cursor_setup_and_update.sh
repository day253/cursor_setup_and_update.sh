...
# File paths
DOWNLOAD_API_URL="https://www.cursor.com/api/download?platform=linux-x64&releaseTrack=stable"
...
if ! command -v jq &> /dev/null; then
    ...
    sudo apt update && sudo apt install -y jq || error "Failed to install jq"
fi

# Check if libfuse2 is installed
if ! dpkg -l | grep -q "libfuse2"; then
    log "Installing required package: libfuse2..."
    sudo apt update && sudo apt install -y libfuse2 || error "Failed to install libfuse2"
fi

# Get download URL from API
DOWNLOAD_URL=$(curl -s "${DOWNLOAD_API_URL}" | jq -r '.downloadUrl')
